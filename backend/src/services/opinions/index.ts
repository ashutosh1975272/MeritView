...
export async function getOpinion(disputeId: string, userId: string): Promise<any> {
  ...
  if (!dispute.opinions) {
    throw new NotFoundError('Opinion not yet generated for this dispute');
  }
  ...
  const content = JSON.parse(decryptedContent) as OpinionContentData;

  logger.info('Opinion retrieved', { disputeId, userId });
  ...
  return {
    ...
    partyAAnalysis: content.partyAAnalysis,
    partyBAnalysis: content.partyBAnalysis,
    comparativeAssessment: content.comparativeAssessment,
    confidenceIndicators: content.confidenceIndicators,
    suggestedConsiderations: content.suggestedConsiderations,
...
export async function generateOpinionPdf(disputeId: string): Promise<Buffer> {
  ...
  try {
    ...
    logger.error('PDF generation failed', error as Error, { disputeId });
    throw new ValidationError('PDF generation failed');
  } finally {
    if (browser) {
      await browser.close();
    }
...
export async function createOpinionFromAggregation(
  ...
  const opinion = await prisma.opinion.create({
    data: {
      disputeId,
      encryptedContent: Buffer.from(encrypted.encryptedContent, 'base64'),
  ...
  try {
    ...
    logger.info('PDF generated for opinion', { disputeId, pdfStorageKey });
  } catch (error) {
    logger.warn('PDF generation skipped', { disputeId, error: String(error) });
  }
...
export async function getOpinionStatus(disputeId: string): Promise<any> {
  const dispute = await prisma.dispute.findUnique({
    where: { id: disputeId },
    include: {
  ...
  const opinion = dispute.opinions;
  const evaluatorOutputs = dispute.evaluatorOutputs;
  const successCount = evaluatorOutputs.filter(e => e.parseSuccess).length;

  if (opinion) {
    return {
      status: 'completed',
      progress: {
        evaluatorsComplete: successCount,
        evaluatorsTotal: evaluatorOutputs.length,
        aggregationStatus: 'completed',
      },
      estimatedCompletion: opinion.deliveredAt,
    };
  }
...
export async function subscribeOpinionStatus(
  disputeId: string,
  userId: string,
  ...
  const poll = async () => {
    while (!stopped) {
      try {
        const status = await getOpinionStatus(disputeId);
        onData(JSON.stringify(status));
...
<style>
  ...
  .header p { font-size: 10pt; color: #555; margin-top: 5px; }
  ...
  .issue .agreement { font-size: 9pt; color: #666; }
  ...
  .analysis-block li { margin-bottom: 3px; }
  ...
  .confidence-item .label { font-size: 9pt; color: #666; margin-top: 5px; }
...
<div class="header">
  ...
  <p>AI-Powered Dispute Analysis &mdash; Generated ${new Date().toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}</p>
</div>
...
<div class="footer">
  <p>MeritView &mdash; AI Decision Support Platform</p>
